...
The repository's `assets/` directory is not part of the bundle. Nothing reads
it at runtime — every cart, rider and rail is a mesh built in code — so
shipping it would only pad the artifact and the image.
